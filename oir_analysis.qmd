...
The per-cell correlations averaged over each cluster, shown the same three ways as the
centroid heatmap. Where a cluster holds one cell type it has one bright class; where it holds
two, the brightness is shared.
...
The same standardized correlations, one panel per reference class. A class with a population

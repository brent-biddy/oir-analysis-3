...
#| fig-width: 4.3
#| fig-height: 3.9
fig, axs = plt.subplots(2, 1, height_ratios=[1, 0.22], constrained_layout=True)
...
#| fig-width: 9
#| fig-height: 3.0
wr_joyal.obs["stratum"] = pd.Categorical(
    wr_joyal.obs["condition"].astype(str) + " " + wr_joyal.obs["timepoint"].astype(str),
    categories=["NORM P14", "NORM P17", "OIR P14", "OIR P17"],
)
strata = list(wr_joyal.obs["stratum"].cat.categories)

# one square window covering the whole embedding, applied to every panel, so a cell sits in
# the same place in all four and the panels can be read against each other
umap_coords = wr_joyal.obsm["X_umap"]
umap_center = umap_coords.mean(axis=0)
umap_half = 0.55 * (umap_coords.max(axis=0) - umap_coords.min(axis=0)).max()

# one scale across the four panels, so brightness means the same thing in each
txn1 = sc.get.obs_df(wr_joyal, keys=["TXN1"])["TXN1"]
txn1_low, txn1_high = txn1.min(), txn1.max()

fig, axs = plt.subplots(2, len(strata), height_ratios=[1, 0.22], constrained_layout=True)
for ax, stratum in zip(axs[0], strata):
    in_stratum = wr_joyal.obs["stratum"] == stratum
    sc.pl.umap(wr_joyal[in_stratum], color="TXN1", ax=ax, frameon=True, colorbar_loc=None,
               vmin=txn1_low, vmax=txn1_high, title=stratum, show=False)
    ax.set_xlim(umap_center[0] - umap_half, umap_center[0] + umap_half)
    ax.set_ylim(umap_center[1] - umap_half, umap_center[1] + umap_half)
    ax.set_aspect("equal", adjustable="box")
    ax.set_title(f"{stratum}  (n={in_stratum.sum():,})", fontsize=9)

for bar_ax in axs[1]:
    bar_ax.axis("off")

colorbar_ax = axs[1][1].inset_axes([0.325, 0.82, 1.35, 0.18])
colorbar_ax.set_in_layout(False)
colorbar = fig.colorbar(axs[0][0].collections[0], cax=colorbar_ax, orientation="horizontal")
colorbar_ticks = np.linspace(txn1_low, txn1_high, 3)
colorbar.set_ticks(colorbar_ticks, labels=[f"{tick:.2f}" for tick in colorbar_ticks])
colorbar.ax.tick_params(labelsize=7)

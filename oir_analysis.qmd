...
#
# there is no threshold to choose. The two scores are compared against each other, which is
# what makes the same rule usable on any cluster listed below.

# the reference names classes and the marker panel names scores; the two vocabularies have
# to be joined by hand
class_scores = {
    "Endothelial": "score_Vascular_Endothelial",
    "Pericyte": "score_Pericytes",
}

# cluster -> (the class the reference called it, the class hiding inside it)
cluster_refinements = {
    "16": ("Endothelial", "Pericyte"),
}
...
for cluster, (parent_class, child_class) in cluster_refinements.items():
    ...
    if child_class not in wr_joyal.obs["cell_type"].cat.categories:
        wr_joyal.obs["cell_type"] = wr_joyal.obs["cell_type"].cat.add_categories([child_class])
    wr_joyal.obs.loc[reassigned, "cell_type"] = child_class

    print(f"cluster {cluster}: {reassigned.sum()} of {in_cluster.sum()} cells "
          f"{parent_class} -> {child_class}")
